<script lang="ts">
  ...
  function updateCell(resolution: string, hpcId: string, patch: Partial<ModelCost>) {
    ...
    const nextCell: ModelCost = {
      ...
      storageTbPerSimMonthByPortfolio:
        patch.storageTbPerSimMonthByPortfolio ?? prev.storageTbPerSimMonthByPortfolio
    };
  ...
  function updateStorage(
    ...
    const prev = getCell(resolution, hpcId);
    const nextStorage = {
      ...prev.storageTbPerSimMonthByPortfolio,
      [portfolio]: value
    };
    updateCell(resolution, hpcId, {
      storageTbPerSimMonthByPortfolio: nextStorage
    });
  }
</script>
...
{#if hpcs.length === 0}
  <p class="text-xs italic text-slate-500" data-testid="cost-matrix-empty">
    Define an HPC first to fill in costs.
  </p>
                ...
                <table class="mt-2 w-full text-xs">
                  <thead>
                    <tr class="text-left text-slate-500">
                      <th class="py-1 font-medium">Data portfolio</th>
                  ...
                  <tbody>
                    {#each dataPortfolios as portfolio (portfolio)}
                      <tr>
                        <td class="py-0.5 font-mono">{portfolio}</td>
                        <td class="py-0.5">
                          <input
                            type="number"
                            min="0"
                            ...
                            value={cell.storageTbPerSimMonthByPortfolio[portfolio] ?? 0}
                            on:input={(e) =>
                              updateStorage(
                                resolution,
                                hpc.id,
